 ...
 *
 * Sample Use:
 *   let generator = new AliasMethod([.1, .2, .3, .4]);
 *   let chosenIndex1 = generator.next();
 *   let chosenIndex2 = generator.next();
 **/

let AliasMethod = function(probabilities){
...
AliasMethod.prototype = {
    ...
    scale : function(probabilities){
        let total = probabilities.reduce(function(a,b) {return a+b;}); 
...
export default function(probabilities){
	let am = new AliasMethod(probabilities);
